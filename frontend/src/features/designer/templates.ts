...
export const WORKFLOW_TEMPLATES: DesignerTemplate[] = [
  ...
  },
  {
    ...
    description: "Busca filas pendientes, añade registros y actualiza estado en una hoja.",
    requiredActivities: [
      "google_sheets.find_rows:v1",
      "google_sheets.append_rows:v1",
      "google_sheets.update_row:v1",
    ],
    build: () => ({
      id: "google-sheets-crud",
      version: "v1",
      displayName: "Google Sheets — CRUD",
      description: "Ejemplo: leer pendientes, insertar pedido y marcar fila como procesada.",
      tags: ["google_sheets", "crud", "spreadsheet"],
      schedule: { type: "instant" },
      scheduledInput: {
        spreadsheetId: "PEGA_AQUI_TU_SPREADSHEET_ID",
        sheetName: "Hoja 1",
        orderId: "ord-001",
        amount: 42,
      },
      inputSchema: {
        type: "object",
        required: ["spreadsheetId", "orderId", "amount"],
        properties: {
          spreadsheetId: { type: "string", description: "ID del spreadsheet" },
          sheetName: { type: "string", description: "Pestaña", default: "Hoja 1" as any },
          orderId: { type: "string", description: "Identificador del pedido" },
          amount: { type: "number", description: "Importe" },
        },
        additionalProperties: false,
      },
      resultSchema: undefined,
      steps: [
        {
          id: "step-find",
          kind: "activity",
          name: "Buscar filas pendientes",
          activityName: "google_sheets.find_rows:v1",
          integrationModule: "google_sheets",
          input: {
        ...
        } as any,
        {
          id: "step-append",
          kind: "activity",
          name: "Añadir fila de pedido",
          activityName: "google_sheets.append_rows:v1",
          integrationModule: "google_sheets",
          input: {
            spreadsheetId: "{{ input.spreadsheetId }}",
            sheetName: "{{ input.sheetName }}",
            rows: [["{{ input.orderId }}", "pending", "{{ input.amount }}"]],
        ...
        } as any,
        {
          id: "step-update",
          kind: "activity",
          name: "Marcar primera pendiente como done",
          activityName: "google_sheets.update_row:v1",
          integrationModule: "google_sheets",
          input: {
            spreadsheetId: "{{ input.spreadsheetId }}",
            sheetName: "{{ input.sheetName }}",
            rowIndex: "{{ steps.step-find.rows[0].rowIndex }}",
            values: { status: "done" },
            valueInputOption: "USER_ENTERED",
          },
          next: null,
