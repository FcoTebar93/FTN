...
export interface DesignerSchedulerDeps {
  ...
  startWorkflow: (name: string, input: unknown, opts?: { correlationId?: string }) => Promise<void>;
...
export async function runScheduledWorkflowTick(deps: DesignerSchedulerDeps): Promise<void> {
  ...
  for (const row of rows) {
    ...
    const correlationId = `sched:${row.subject}:${row.id}:${now.getTime()}`;
    try {
      await deps.startWorkflow(row.runtimeName, row.payload.scheduledInput ?? {}, { correlationId });
      ...
      deps.log.error("designer.scheduler", { subject: row.subject, id: row.id, err: String(e), correlationId });
    }
  }
}
