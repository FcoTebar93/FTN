...
let sharedFromUrl: Redis | null = null;

function clientFromUrl(redisUrl: string): Redis {
  if (!sharedFromUrl) {
    sharedFromUrl = new Redis(redisUrl, { maxRetriesPerRequest: 2, lazyConnect: false });
  }
...
export function redisPublishActivityDefinition(config: MessagingConfig): ActivityDefinition<RedisPublishInput, RedisPublishResult> {
  const injected = config.redis;
  const url = config.redisUrl?.trim();
  if (!injected && !url) {
    throw new Error("Config inválida para messaging.redisPublish: falta redis o redisUrl");
  }
  ...
  return {
    ...
    async execute(input: RedisPublishInput, ctx: ActivityExecutionContext): Promise<RedisPublishResult> {
      ...
      const redis = injected ?? clientFromUrl(url!);
